...
add column if not exists turn_stage text not null default 'awaiting_draw';
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'games_turn_stage_check'
      and connamespace = 'rummy500'::regnamespace
  ) then
    alter table rummy500.games
    add constraint games_turn_stage_check check (turn_stage in ('awaiting_draw', 'awaiting_discard'));
  end if;
end
$$;
...
begin
  ...
  if normalized_action = 'draw_stock' then
    ...
    if current_hand_count <> cards_per_player then
      raise exception 'The current hand is not ready for a draw';
    end if;
